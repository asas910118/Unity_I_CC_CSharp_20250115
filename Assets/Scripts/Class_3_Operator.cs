...
public class Class_3_Operator : MonoBehaviour
{
    ...
    // [標題(標題文字)] :在屬性面板上提示標題文字
    [Header("等級")]
    public int lv = 1;
    // [提示(提示文字)] : 在屬性面板滑鼠停留在變數上時顯示
    [Tooltip("這是角色的移動速度，不建議超過 100。")]
    public float moveSpead = 2.5f;
    // [範圍(最小，最大)]:幫數值類型添加範圍 (面板上改為滑桿)
    [Range(1, 100)]
    public byte count = 10;
    // 範圍不能使用在非數值類型上，會顯示 "Use Range With fioat or int."
    [Range(0, 10)]
    public string weapon = "匕首";
    // [文字範圍 (最小行，最大行)] : 使用在字串上，設定面板上的文字範圍
    [TextArea(2, 5)]
    ...
    // 2. 使用快速關鍵字完成 ， 例如：輸入 Awake 挑選要使用的事件按 Enter
